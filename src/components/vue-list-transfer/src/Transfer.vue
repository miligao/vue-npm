  ...
  <div class="transferContainer" :style="{ width: width, height: height }">
    <div class="transferPanel">
      ...
      <ul class="body" @scroll="scroll">
        <li :class="['item', { select: item.select , disable: item.disabled }]" v-for="(item, index) of checkList" @click="checkListClick(item, index)" :key="'checkList' + item.key">
    ...
    <div class="transferPanel">
      ...
      <div class="searchBar" v-if="showSearch">
        <img src="./assets/search.png" alt="icon" class="searchIcon">
        <input type="text" v-model.trim="rightSearch" @keyup.enter="rightSearchBlur" @blur="rightSearchBlur" :placeholder="placeholder" class="searchInput">
      </div>
      <ul class="body">
        <li :class="['item', { select: item.select , disable: item.disabled }]" v-for="(item, index) of rightListSource" @click="checkedListClick(item, index)" :key="'checkedList' + item.key">
          <img :src="item | checkFilter" alt="check" class="icon">
          <p>{{ item.label }}</p>
        </li>
        ...
        if (this.defaultChecked) {
          if (this.checkedList.length === 0) {
            ...
            this.checkList.map((ele, index) => {
              if (this.defaultChecked.some(item => item.label === ele.label)) ele.select = true
            })
            // 右边栏数据填充
            ...
            // 左边栏分页选中
            this.checkList.map((ele, index) => {
              if (this.checkedList.some(item => item.label === ele.label)) ele.select = true
            })
          }
        }
      },
    ...
    methods: {
      ...
      setSearchCheckedList () {
        ...
        this.checkedList.map((ele, index) => {
          if (ele.label.indexOf(this.rightSearch) !== -1) this.searchCheckedList.push(ele)
        })
      },

      checkListClick (item, index) {
        if (item.disabled) return
        if (item.select) {   // 左边栏取消选中
          item.select = false
          this.checkedList.map((ele, index) => {   // 右边栏删除对应数据
      ...
      checkedListClick (item, index) {
        if (this.rightIsSearch) {   // 右边栏是搜索结果
        ...
        this.checkList.map((ele, index) => {   // 左边栏取消数据选中
          if (ele.label === item.label) {
            ele.select = false
            return false
          }
      ...
      addAll () {
        this.checkList.map((ele, index) => {
          if (!ele.disabled && !ele.select) {   // 过滤掉 禁选项 和 已选项
            ele.select = true
            this.checkedList.push({ ...ele })
          }
        })
      ...
      removeAll () {
        ...
        this.dataSource.map((ele, index) => {   // 初始化源数据
          ele.select = false
        })
